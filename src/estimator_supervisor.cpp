...
Supervisor::Supervisor(ros::NodeHandle &nh, double &window, double &max_norm,
                       std::string &topic, std::string &msg_type)
    : nh_(nh), window_(window), max_norm_(max_norm), topic_(topic),
      msg_type_(msg_type) {

  // Advertise supervisor service
  srv_ = nh_.advertiseService("service/supervision",
                              &Supervisor::serviceHandler, this);
}

bool Supervisor::serviceHandler(std_srvs::Trigger::Request &,
  ...
  if (msg_type_ == "posestamped") {
    sub_estimate_ =
        nh_.subscribe(topic_, 1, &Supervisor::estimateCallback, this);
  } else if (msg_type_ == "posewithcovariancestamped") {
    sub_estimate_ = nh_.subscribe(
        topic_, 1, &Supervisor::estimateWithCovarianceCallback, this);
  }

  // Wait till the window gets filled by measurements
  while (!buffer_full_) {
    std::cout << YELLOW("Waiting for filling the estimate buffer...")
              << std::endl;
...
bool Supervisor::estimatorSupervision() {
  ...
  // Safety check
  if (position_buffer_.front().timestamp <= 0.0 ||
      position_buffer_.back().timestamp <= 0.0) {
  ...
  if ((position_buffer_.back().p - position_buffer_.front().p).norm() >
      max_norm_) {
    return false;
  }
...
void Supervisor::estimateWithCovarianceCallback(
  ...
  data.p << msg->pose.pose.position.x, msg->pose.pose.position.y,
...
void Supervisor::estimateCallback(
